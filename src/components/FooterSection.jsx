...
function LocationMap() {
  ...
  // useRef = a box to store a value that survives across renders, changing it does not cause component to re-rerender

  // null is the current value, we say null because there is no element when the component first runs
  const mapInstance = useRef(null);

  useEffect(
    () => {
      if (mapInstance.current || !mapRef.current) return;
      // if the map already exists or the div isnt ready - stop! this prevent building the map 2x!
      ...
      // store the map so it survuves rerenders and can be controlled later
      return () => {
        map.remove();
...
export default function FooterSection() {
  return (
    <footer className="footer">
      <div className="mx-auto max-w-7xl px-6 pb-6 pt-16 sm:pt-24 lg:px-8 lg:pt-32 2xl:max-w-400">
        {/* Map */}
        ...
        {/* Footer Columns */}
        <div className="mt-15 grid grid-cols-2 gap-8 md:grid-cols-4">
